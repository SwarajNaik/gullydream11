...
export default function CreateTeamPage() {
  ...
  if (store.step === 2) {
    return (
      <div className="min-h-screen bg-[#1A1A2E]">
        ...
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-[#16213E] border-t border-[#2A2A4A]">
          <Button
            className="w-full"
            size="lg"
  ...
  return (
    <div className="min-h-screen bg-[#1A1A2E]">
      {/* Header */}
      <div className="bg-[#0F3460] p-4">
        <button onClick={() => { store.reset(); router.back(); }} className="text-[#9E9E9E] mb-2">
          ← Back
        </button>
      ...
      {/* Bottom Bar */}
      <div className="fixed bottom-0 left-0 right-0 flex items-center justify-between p-4 bg-[#16213E] border-t border-[#2A2A4A]">
        <div>
          <p className="text-xs text-[#9E9E9E]">Players</p>
          <p className="text-sm font-bold">{store.selectedPlayers.length}/11</p>
        </div>
        <div>
          <p className="text-xs text-[#9E9E9E]">Credits Used</p>
          <p className="text-sm font-bold">{totalCredits.toFixed(1)}/100</p>
        </div>
        <Button size="sm" disabled={store.selectedPlayers.length !== 11} onClick={() => store.setStep(2)}>
          NEXT →
        </Button>
      </div>
    </div>
